...
def mapear_imagenes_s3_propiedades():
    """
    SOLUCIÓN INTEGRADA: Mapea imágenes S3 reales a propiedades PostgreSQL
    Esta función se ejecuta automáticamente en el flujo de procesamiento
    """
    try:
        logger.info("🔄 INICIANDO MAPEO AUTOMÁTICO IMÁGENES S3...")
        
        # 1. Obtener imágenes disponibles en S3
        s3 = boto3.client('s3')
        imagenes_s3 = []
        ...
        for page in paginator.paginate(Bucket=S3_BUCKET):
            if 'Contents' in page:
                for obj in page['Contents']:
                    key = obj['Key']
                    if key.endswith('.jpg') and 'cuernavaca-2025-' in key:
                        try:
                            filename = key.split('/')[-1]
                            if 'cuernavaca-2025-' in filename:
                                # Extraer ID: cuernavaca-2025-05-29-1234567890.jpg
                                id_parte = filename.split('cuernavaca-2025-')[1].split('.jpg')[0]
                                id_imagen = id_parte.split('-', 3)[-1]  # Último segmento
                                
                                imagenes_s3.append({
                                    'key': key,
                                    'url': f"{S3_BASE_URL}{key}",
                                    'id_extraido': id_imagen,
                                    'filename': filename
                                })
                        except Exception as e:
                            continue
        
        logger.info(f"✅ {len(imagenes_s3)} imágenes S3 encontradas")
        
        # 2. Conectar a PostgreSQL y mapear
        conn = psycopg2.connect(**DB_CONFIG)
        cur = conn.cursor()
        
        actualizadas = 0
        
        for img in imagenes_s3:
            prop_id = img['id_extraido']
            imagen_url = img['url']
            
            # Actualizar PostgreSQL con URL real
            cur.execute("""
                UPDATE propiedades 
                SET imagenes = %s 
                WHERE id = %s
        ...
        logger.info(f"✅ MAPEO COMPLETADO: {actualizadas} propiedades con imágenes S3 reales")
        return True
        
    except Exception as e:
        logger.error(f"❌ Error en mapeo automático S3: {e}")
        return False
...
def procesar_numero_mexicano(texto: str) -> Optional[float]:
    ...
    patrones_precio_definititvos = [
        ...
        # EMOJIS CON NÚMEROS
        (r'💰💲([0-9,.\s]+)', lambda x: procesar_numero_base_script(x)),
        (r'💲([0-9,.\s]+)', lambda x: procesar_numero_base_script(x)),
        
        # PRECIOS CON DESCRIPCIÓN
        (r'precio[:\s;]*\$?([0-9,.\s]+)', lambda x: procesar_numero_base_script(x)),
        (r'desde[:\s]*\$?([0-9,.\s]+)', lambda x: procesar_numero_base_script(x)),
        
        # FORMATO ESTÁNDAR CON SÍMBOLOS
        (r'\$([0-9,.\s]+)', lambda x: procesar_numero_base_script(x)),
        (r'([0-9,.\s]+)\s*(?:pesos|MXN|mxn)', lambda x: procesar_numero_base_script(x)),
...
def procesar_numero_base_script(texto: str) -> Optional[float]:
    """
    Procesa un número base limpiando separadores.
    FUNCIÓN AUXILIAR para el script de procesamiento.
...
def extraer_tipo_operacion(texto, precio=None):
    """
    Extrae el tipo de operación con lógica inteligente mejorada.
    
    REGLAS INTELIGENTES:
    1. Si descripción tiene palabras clave claras → usar descripción
    2. Si precio > $300,000 sin palabras clave → "venta"
    3. Si precio ≤ $50,000 sin palabras clave → "renta"
    4. Si precio $50,000-$300,000 sin palabras clave → None (desconocido)
    """
    if not texto:
        # Solo clasificar por precio si no hay texto
        if precio:
            if precio > 300000:
                return "venta"
            elif precio <= 50000:
    ...
    # Detectar venta con alta confianza
    patrones_venta = [
    ...
    # Detectar renta con alta confianza
    patrones_renta = [
    ...
    # Si no hay palabras clave claras, clasificar por precio con lógica inteligente
    if precio:
        if precio > 300000:  # Precios altos son ventas
            return "venta"
        elif precio <= 50000:  # Precios bajos son rentas
            return "renta"
        # Precios medios (50K-300K) quedan como desconocido para evitar errores
...
def procesar_datos_crudos(archivo_entrada: str, archivo_salida: str) -> None:
    ...
    try:
            ...
            try:
                ...
                if not tipo_op and precio_info:
                    try:
                        ...
                        # UMBRALES CORREGIDOS PARA 2025:
                        # Ventas típicas: >$300,000 (casas desde 300K)
                        # Rentas típicas: $5,000-$150,000 mensual
                        if valor >= 300_000:  # Si es mayor a 300 mil, probablemente es venta
                            tipo_op = "venta"
                        elif 5_000 <= valor <= 150_000:  # Rango típico de rentas mensuales en México 2025
                            tipo_op = "renta"
                        # Para valores entre 150K-300K, no asumir nada (dejar que lo determine el texto)
                    except:
                        pass
        ...
        # ✅ INTEGRACIÓN AUTOMÁTICA: Mapear imágenes S3 después de procesar datos
        logger.info("\n🔄 EJECUTANDO MAPEO AUTOMÁTICO IMÁGENES S3...")
        mapeo_exitoso = mapear_imagenes_s3_propiedades()
        if mapeo_exitoso:
            logger.info("✅ MAPEO S3 INTEGRADO EXITOSAMENTE AL FLUJO PRINCIPAL")
        else:
            logger.warning("⚠️ MAPEO S3 TUVO PROBLEMAS - REVISAR LOGS")
